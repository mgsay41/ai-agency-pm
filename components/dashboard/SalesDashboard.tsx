...
import { SalesDashboardData } from "@/lib/dashboard/sales-dashboard";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:gap-5 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatsCard
          title="Pipeline"
          value={data.recentClients.length}
      ...
      <Card className="border-[#E5E5E5]">
        ...
        <CardContent>
          <div className="space-y-3">
            {data.myProjectsList.length === 0 ? (
              <p className="text-sm text-[#A3A3A3] text-center py-4">
                No projects yet
              </p>
            ) : (
              data.myProjectsList.map((project) => (
                <div
                  key={project.id}
                  className="flex items-start justify-between p-3 bg-[#FAFAFA] rounded hover:bg-[#F5F5F5] transition-colors"
                >
                ...
                >
                  ...
                  {meeting.project && (
                    <div className="space-y-1 ml-6">
                      ...
                      <div className="flex items-center gap-2">
                        <Building2 className="h-3 w-3 text-[#A3A3A3]" />
                        <p className="text-xs text-[#A3A3A3]">
                          {meeting.project.client.companyName}
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  {meeting.agenda && (
                    <p className="text-xs text-[#A3A3A3] ml-6 mt-2 line-clamp-2">
                      {meeting.agenda}
                    </p>
                  )}
                </div>
